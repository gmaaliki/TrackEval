...
class SoccernetMotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, 'data/gt/soccernet_mot_challenge/'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/soccernet_mot_challenge/'),  # Trackers location
            'OUTPUT_FOLDER': None,  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval (if None, all in folder)
            'CLASSES_TO_EVAL': ['ball', 'goalkeeper', 'player', 'referee'],  # Valid: ['ball', 'goalkeeper', 'player', 'referee']
            'BENCHMARK': 'SoccerNet',  # Valid: 'SoccerNet'
    ...
    def __init__(self, config=None):
        ...
        # Get classes to eval
        self.valid_classes = ['ball', 'goalkeeper', 'player', 'referee']
        ...
        self.class_name_to_class_id = {'ball': 0, 'goalkeeper': 1, 'player': 2, 'referee': 3}
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        For 4 classes: {'ball': 0, 'goalkeeper': 1, 'player': 2, 'referee': 3}
        """
        # Check that input data has unique ids
        self._check_unique_ids(raw_data)

        # All classes avaliable in soccernet detection
        valid_classes = {'ball': 0, 'goalkeeper': 1, 'player': 2, 'referee': 3}
        
        distractor_classes = []  # No distractor class available
        
        cls_id = valid_classes[cls]
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            if self.do_preproc and gt_ids.shape[0] > 0 and tracker_ids.shape[0] > 0:
                ...
                valid_class_numbers = list(valid_classes.values())
                invalid_classes = np.setdiff1d(np.unique(gt_classes), valid_class_numbers)
                ...
                # Only apply distractor class removal if there are any distractor classes defined
                if len(distractor_classes) > 0:
                    is_distractor_class = np.isin(gt_classes[match_rows], distractor_classes)
                    to_remove_tracker = match_cols[is_distractor_class]

            # Only keep detections that:
            # 1. Are not marked as to be removed (zero_marked)
            # 2. Match the current class being evaluated
            if self.do_preproc:
                tracker_to_keep_mask = np.equal(tracker_classes, cls_id)
            else:
                # If preprocessing is disabled, only filter by zero_marked
            ...
            similarity_scores = similarity_scores[:, tracker_to_keep_mask]

            # Only keep ground truth detections that:
            # 1. Are not marked as to be removed (zero_marked)
            # 2. Match the current class being evaluated
            if self.do_preproc:
